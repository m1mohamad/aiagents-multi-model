#!/usr/bin/env python3
"""
Groq API Implementation with Conversation History
Uses Llama 3.3 70B via Groq's ultra-fast infrastructure
...
# Configuration
CONTEXT_DIR = Path("/ai/groq/context")
HISTORY_DIR = Path("/ai/groq/history")
...
def decrypt_api_key():
    """Decrypt Groq API key using age"""
    try:
...
def chat(message, context_name=None, project_name=None, max_history=10):
    ...
    # Build messages array for API (OpenAI-compatible format)
    messages = []
    for msg in history:
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print("  groq-chat \"message\"                    # Use current context")
        print("  groq-chat --context NAME \"message\"      # Use specific context")
        print("  groq-chat --project NAME \"message\"      # Use project context")
        print("  groq-chat --list                        # List contexts")
        print("  groq-chat --switch CONTEXT              # Switch context")
        sys.exit(1)
