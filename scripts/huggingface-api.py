#!/usr/bin/env python3
"""
HuggingFace API Implementation with Conversation History
Uses Llama 3.1 70B via HF Inference API
...
# Configuration
CONTEXT_DIR = Path("/ai/huggingface/context")
...
def decrypt_api_key():
    """Decrypt HuggingFace API key using age"""
    try:
...
def chat(message, context_name=None, project_name=None, max_history=10):
    """Send message to HuggingFace with conversation history
    ...
    try:
        ...
        print(f"Error calling HuggingFace API: {e}", file=sys.stderr)
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print("  huggingface-chat \"message\"                    # Use current context")
        print("  huggingface-chat --context NAME \"message\"      # Use specific context")
        print("  huggingface-chat --project NAME \"message\"      # Use project context")
        print("  huggingface-chat --list                        # List contexts")
        print("  huggingface-chat --switch CONTEXT              # Switch context")
        sys.exit(1)
