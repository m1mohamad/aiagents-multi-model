...
test:
	...
	@echo ""
	@echo "Testing Groq (Llama 3.3 70B - FREE):"
	@sudo podman exec groq-agent /home/agent/groq-chat "Hello" || echo "✗ Groq failed"
	@echo ""
	@echo "Testing HuggingFace (Llama 3.1 70B - FREE):"
	@sudo podman exec huggingface-agent /home/agent/huggingface-chat "Hello" || echo "✗ HuggingFace failed"
	@echo ""
	@echo "Testing Gemini (1.5 Flash):"
	@sudo podman exec gemini-agent /home/agent/gemini-chat "Hello" || echo "✗ Gemini failed"
	@echo ""
	@echo "Testing Grok (xAI):"
...
status:
	...
	@test -f /ai/groq/context/.secrets.age && echo "✓ Groq configured (FREE)" || echo "✗ Groq not configured"
	@test -f /ai/huggingface/context/.secrets.age && echo "✓ HuggingFace configured (FREE)" || echo "✗ HuggingFace not configured"
...
contexts:
	...
	@echo ""
	@echo "=== Groq Contexts (FREE) ==="
	@test -d /ai/groq/history && ls -1 /ai/groq/history | grep -v "^\\." || echo "No contexts found"
	@echo ""
	@echo "=== HuggingFace Contexts (FREE) ==="
	@test -d /ai/huggingface/history && ls -1 /ai/huggingface/history | grep -v "^\\." || echo "No contexts found"
	@echo ""
...
# Show container logs
logs:
	@echo "Enter container name (claude-agent, groq-agent, huggingface-agent, grok-agent, or gemini-agent):"
